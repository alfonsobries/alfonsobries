...
export default function ManageBehaviorsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const next = await fetchBehaviors(route, kid);
      writeCache(cacheKeys.behaviors(kid), next);
      setBehaviors(next);
    } catch (error) {
      // The list stays readable offline; editing it still needs the API.
      const cached = isOfflineError(error) ? readCache<Behavior[]>(cacheKeys.behaviors(kid)) : null;
