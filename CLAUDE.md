...
- **Auth**: Sign in with Apple only. `AuthProvider` (`app/src/api/auth.tsx`) exchanges the Apple identity token at `POST /api/auth/apple` for a Sanctum bearer token, persisted in `expo-secure-store`. The root layout gates routes with `Stack.Protected`: the `login` welcome screen when signed out, the `(app)` tab group when signed in. The stored token keeps the session across launches with no network — only an API response that rejects it signs the user out. Credential setup: `docs/apple-sign-in.md`
- **Offline**: the app is used away from wi-fi, so reads render from an on-disk cache and writes the device can decide on its own are queued and replayed on reconnect (`app/src/offline/`). Conventions in `app/.claude/agents/offline.md`
